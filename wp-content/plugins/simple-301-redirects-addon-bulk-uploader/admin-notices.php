<?php
	...
	class S301B_Admin_Notices {
		...
		public function admin_notices_suggestions() {
			...
			?>
            <div class="<?php echo esc_attr( $this->prefix ); ?>-dismiss-suggestions wbcr-s301bu-notice-wrapper notice notice-success is-dismissible" data-nonce="<?php echo esc_attr( wp_create_nonce( $this->prefix . '-dismiss-suggestions' ) ); ?>">
				<?php if ( ! empty( $this->suggestions_message ) ): ?>
				...
				<?php foreach ( $this->missing as $plugin ) : ?>
                    ...
                    <div class='wbcr-s301bu-notice'>
                        <div class='wbcr-s301bu-leftcol'>
                            <img class='wbcr-s301bu-plugin-ico' src='<?php echo $this->suggestions[ $plugin ]['ico']; ?>' alt='<?php echo $plugin; ?>'>
                        </div>
                        <div class='wbcr-s301bu-content'>
                            <div class='wbcr-s301bu-title'><?php echo $this->suggestions[ $plugin ]['name']; ?></div>
                            <div class="wbcr-s301bu-text-cols">
                                <div class='wbcr-s301bu-description'><?php echo $this->suggestions[ $plugin ]['desc']; ?></div>
                                <div class='wbcr-s301bu-rightcol'>
                                    <a class='wbcr-s301bu-install-btn' href='<?php echo esc_url( $this->get_install_url( $plugin ) ); ?>'>
										<?php
											// Check existing plugin
											$exists = @file_exists( WP_PLUGIN_DIR . '/' . $plugin );
											
											if ( $exists && ! is_plugin_active( $this->plugin_file ) ) {
												echo 'Activate';
											} else {
												echo 'Install and optimize';
											}
										?>
                                    </a>
                                </div>
