...
if (!class_exists('WPSE_Sheet_Factory')) {
	...
	class WPSE_Sheet_Factory {
		...
		function add_account_link($post_type) {
			...
			?>
			<a href="<?php echo $this->args['fs_object']->get_account_url(); ?>" class="button" target="_blank"><i class="fa fa-user"></i> <?php _e('My license', VGSE()->textname); ?></a>
			<?php
		}

		function disable_sql_formulas_for_serialized_columns__premium_only($allowed, $formula, $column, $post_type) {
			if ($post_type === $this->get_prop('post_type') && !empty($this->get_prop('serialized_columns')) && in_array($column['key'], $this->get_prop('serialized_columns'))) {
				$allowed = false;
			}
		...
		function append_post_type_to_post_types_list($post_types, $args, $output) {
			...
			if (isset($post_types[$this->get_prop('post_type')]) || !empty(wp_list_filter($post_types, array('name' => $this->get_prop('post_type'))))) {
				return $post_types;
			}
